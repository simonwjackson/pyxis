...
describe("isHealthRequest", () => {
	it("matches only GET /healthz", () => {
		expect(isHealthRequest(new URL("http://pyxis.local/healthz"), "GET")).toBe(true);
		expect(isHealthRequest(new URL("http://pyxis.local/healthz"), "POST")).toBe(false);
		expect(isHealthRequest(new URL("http://pyxis.local/healthz/"), "GET")).toBe(false);
		expect(isHealthRequest(new URL("http://pyxis.local/"), "GET")).toBe(false);
		expect(isHealthRequest(new URL("http://pyxis.local/trpc/healthz"), "GET")).toBe(false);
	});
});
