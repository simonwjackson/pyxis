...
function useViewportCardSize() {
	const [size, setSize] = useState(() => computeViewportCardSize());
	useEffect(() => {
		const onResize = () => setSize(computeViewportCardSize());
...
function useDetailSize() {
	const [size, setSize] = useState(() => computeDetailSize());
	useEffect(() => {
		const onResize = () => setSize(computeDetailSize());
		window.addEventListener("resize", onResize);
		return () => window.removeEventListener("resize", onResize);
	}, []);
	return size;
		...
		>
			...
			>
				{tracks.map((track, index) => (
					// biome-ignore lint/a11y/useSemanticElements: cards are absolute-positioned rich media tiles; native button layout would distort the prototype.
					<div
						key={track.id}
						role="button"
			...
			{/* ── Detail View ─────────────────────────────────────────── */}
			{/* biome-ignore lint/a11y/useSemanticElements: this full-screen rich detail surface acts like a dismissable overlay, not a native button. */}
			<div
				role="button"
