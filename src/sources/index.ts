  ...
  return {
    ...
    async getPlaylistTracks(sourceType, playlistId) {
      ...
      if (!source || !hasPlaylistCapability(source)) {
        throw new Error(`Source "${sourceType}" does not support playlists`);
      }
    ...
    async getAlbumTracks(sourceType, albumId) {
      ...
      if (!source || !hasAlbumCapability(source)) {
        throw new Error(`Source "${sourceType}" does not support album tracks`);
      }
    ...
    async searchAll(query: string): Promise<SearchResult> {
      ...
      const metadataSearchable = metadataSources.filter(
        hasMetadataSearchCapability,
      );
      ...
      const mergedAlbums = matcher.getAll().map(normalizedToCanonicalAlbum);

      const _totalMs = Date.now() - primaryStart;
