...
- [ ] 1.1 Install `@proseql/core` (npm) and `@proseql/node` (GitHub: `github:simonwjackson/proseql#main`). Run `just nix-lock` to regenerate bun.nix. Verify ProseQL + NodeStorageLayer works with Bun runtime (create minimal test: init db, write YAML, read back, then remove test file).
