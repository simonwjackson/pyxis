...
function formatSettingsOutput(
  ...
  const lines = [
    ...
    formatBoolean("Explicit Content Filter", settings.isExplicitContentFilterEnabled),
...
export function registerSettingsCommand(program: Command): void {
  program
    ...
    .action(async (options, command: Command) => {
      ...
      const effect: Effect.Effect<GetSettingsResponse, PandoraError> = Effect.gen(function* () {
        ...
        return yield* getSettings(session!)
      })
