...
function formatGenresTable(
	...
	if (options.json) {
		const response = {
			...
			data: {
				...
				totalStations: categories.reduce(
					(sum, cat) => sum + cat.stations.length,
					0,
				),
			},
		};
	...
	const totalStations = categories.reduce(
		(sum, cat) => sum + cat.stations.length,
		0,
	);
...
export function registerGenresCommand(program: Command): void {
	program
		...
		.action(async (options: GenresOptions, command: Command) => {
			...
			const effect: Effect.Effect<GenreStationsResult, PandoraError> =
				Effect.gen(function* () {
					const response: GetGenreStationsResponse = yield* withSession(
						(session) => getGenreStations(session),
