...
export async function getVideoInfo(videoId: string): Promise<YtDlpVideoInfo> {
  ...
  const data = JSON.parse(output) as {
    ...
    duration?: number;
    chapters?:
      | readonly { title: string; start_time: number; end_time: number }[]
      | null;
  };
  return {
 ...
 * @throws Error if yt-dlp fails or video is unavailable
 */
export async function getTrackInfo(videoId: string): Promise<YtDlpTrackInfo> {
  const output = await runYtDlp([
    "--dump-json",
    "--no-playlist",
    `https://music.youtube.com/watch?v=${videoId}`,
 ...
 */
export async function getAlbumEntries(albumUrl: string): Promise<{
  readonly id: string;
  readonly title: string;
  readonly artist: string;
  readonly entries: readonly YtDlpPlaylistEntry[];
  ...
  const output = await runYtDlp(["--dump-json", "--flat-playlist", albumUrl]);
  ...
  for (const line of lines) {
    ...
    if (typeof record.playlist_title === "string")
      albumTitle = record.playlist_title;
    if (typeof record.playlist_uploader === "string")
      albumArtist = record.playlist_uploader;
    if (typeof record.thumbnail === "string" && !albumThumbnail)
      albumThumbnail = record.thumbnail;
    const id = typeof record.id === "string" ? record.id : undefined;
    const title = typeof record.title === "string" ? record.title : undefined;
    // Skip entries without valid id or title (unavailable/private videos)
    if (!id || !title) continue;
...
export async function getPlaylistEntries(
  ...
  for (const line of lines) {
    ...
    if (typeof record.playlist_title === "string")
      playlistTitle = record.playlist_title;
    const id = typeof record.id === "string" ? record.id : undefined;
    const title = typeof record.title === "string" ? record.title : undefined;
    // Skip entries without valid id or title (unavailable/private videos)
    if (!id || !title) continue;
