			...
			> = Effect.gen(function* () {
				...
				if (!station) {
					return yield* Effect.fail(
						new SessionError({
							message:
								"Station not found. Use 'pandora stations list' to see available stations.",
						}),
					);
				}

				const newStation: TransformSharedStationResponse =
					yield* transformSharedStation(session, {
