...
function formatDeleteSuccess(
  ...
  return pc.green(`${capitalizeFirst(bookmarkType)} bookmark deleted successfully!`)
}
...
export function registerDeleteCommand(program: Command): void {
  program
    ...
    .requiredOption(
      "-t, --type <type>",
      "Bookmark type: artist or song"
    )
    .action(async (bookmarkToken: string, options: DeleteOptions, command: Command) => {
      ...
      const effect: Effect.Effect<Record<string, never>, PandoraError> =
        Effect.gen(function* () {
