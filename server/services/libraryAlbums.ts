...
export async function resolveAlbumStatesForSourceIds(
  ...
  const refsBySourceId = new Map(
    refs.map(
      (ref) =>
        [formatSourceId(ref.source as SourceType, ref.sourceId), ref] as const,
    ),
  );
...
export async function saveAlbumToLibrary(
  ...
  if (existingRef) {
    const existingAlbum = (await db.albums.findById(existingRef.albumId)
      .runPromise) as AlbumRecord | null;
    if (!existingAlbum) {
      throw new Error(
        `Album source ref points to missing album: ${existingRef.albumId}`,
      );
    }
