...
import { getCacheDir, getSessionCachePath, getSessionLockPath, ensureCacheDir } from "./paths.js"
...
const isExpired = (cached: CachedSession): boolean => {
...
export const getSessionInfo = async (): Promise<SessionInfo | null> => {
  ...
  try {
    ...
    const expiresIn = expired ? 0 : Math.floor((cached.expiresAt - Date.now()) / 1000)
