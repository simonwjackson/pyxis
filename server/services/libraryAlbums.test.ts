...
function withTempFiles(baseDir: string) {
	return {
		...
		albumSourceRefs: { ...dbConfig.albumSourceRefs, file: join(baseDir, "album-source-refs.yaml") },
		albumTracks: { ...dbConfig.albumTracks, file: join(baseDir, "album-tracks.yaml") },
		playlists: { ...dbConfig.playlists, file: join(baseDir, "playlists.yaml") },
		playerState: { ...dbConfig.playerState, file: join(baseDir, "player-state.yaml") },
		queueState: { ...dbConfig.queueState, file: join(baseDir, "queue-state.yaml") },
		listenLog: { ...dbConfig.listenLog, file: join(baseDir, "listen-log.jsonl") },
		trackSources: { ...dbConfig.trackSources, file: join(baseDir, "track-sources.yaml") },
		upgradeQueue: { ...dbConfig.upgradeQueue, file: join(baseDir, "upgrade-queue.yaml") },
	} as const;
}
...
describe("saveAlbumToLibrary", () => {
	...
	it("re-adding a dismissed album restores it to discovery", async () => {
		...
		try {
			...
			const result = await saveAlbumToLibrary(db, sourceManager, "ytmusic:remote_album_1", { now: 250 });
			expect(result).toEqual({
				id: "album_1",
				outcome: "restored",
				placement: "discovery",
			...
			const restored = await listLibraryAlbums(db, { placements: ["discovery"] });
...
describe("placement-aware listing", () => {
	...
	it("setAlbumPlacement moves albums between placements", async () => {
		...
		try {
			...
			const updated = await setAlbumPlacement(db, "album_1", "archive", { now: 999 });
