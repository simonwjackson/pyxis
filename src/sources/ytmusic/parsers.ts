...
export const parseAlbumSearchResults = (
  ...
  return contents
    .map((item): ParsedAlbum | null => {
      ...
      const albumName = listItem?.flexColumns?.[0]
        ?.musicResponsiveListItemFlexColumnRenderer?.text?.runs?.[0]?.text as
        | string
        | undefined;
...
const parseFromTrackFallback = (
  ...
  const firstSection = twoColumn?.secondaryContents?.sectionListRenderer
  ...
  const shelf = (firstSection?.musicShelfRenderer ??
    ...
    | undefined;
  const firstTrack = shelf?.contents?.[0]?.musicResponsiveListItemRenderer;
