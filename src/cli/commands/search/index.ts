...
function formatSearchResults(
  ...
  if ((options.type === "all" || options.type === "artist") && response.artists) {
    results.push(
  ...
  if ((options.type === "all" || options.type === "genre") && response.genreStations) {
    results.push(
...
export function registerSearchCommand(program: Command): void {
  program
    ...
    .action(async (query: string, options: SearchOptions, command: Command) => {
      const globalOpts: GlobalOptions = command.optsWithGlobals() as GlobalOptions
      ...
      if (!validTypes.includes(allOpts.type)) {
        ...
        console.error(
          pc.red(
            `Invalid type: ${allOpts.type}. Must be one of: ${typeList}`
          )
        )
        process.exit(2)
      }

      const effect: Effect.Effect<SearchResult, PandoraError> = Effect.gen(function* () {
        ...
        const response: MusicSearchResponse = yield* search(session, query)
