...
const calculateSyncTime = (encryptedTime: string): Effect.Effect<number, PartnerLoginError> =>
  decrypt(ANDROID_DEVICE.decryptKey)(encryptedTime).pipe(
    ...
    Effect.mapError((e) => new PartnerLoginError({
...
> =>
  Effect.gen(function* () {
    const params = new URLSearchParams({ method: "auth.partnerLogin" })
    ...
    if (json.stat !== "ok") {
      return yield* Effect.fail(
        new PartnerLoginError({ message: "Partner login failed" })
      )
    }
...
export const userLogin = (
  ...
  Effect.gen(function* () {
    ...
    const body = yield* encryptJson(ANDROID_DEVICE.encryptKey)(payload).pipe(
      Effect.mapError((e) => new UserLoginError({
    ...
    if (json.stat !== "ok") {
      return yield* Effect.fail(
        new UserLoginError({ message: "User login failed" })
      )
    }
