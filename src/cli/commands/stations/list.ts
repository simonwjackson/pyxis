...
function formatStationsTable(
	...
	if (stations.length === 0) {
		return pc.dim(
			"No stations found. Create one with: pandora stations create",
		);
	}
...
export function registerListCommand(program: Command): void {
	program
		...
		.action(async (options: ListOptions, command: Command) => {
			...
			const effect: Effect.Effect<StationListResult, PandoraError> = Effect.gen(
				function* () {
					const response: StationListResponse = yield* withSession(
						(session) => getStationList(session),
