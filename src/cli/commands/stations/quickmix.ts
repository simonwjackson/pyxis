...
export function registerQuickMixCommands(program: Command): void {
	...
	quickmix
		...
		.argument(
			"<station-names...>",
			"Station names or tokens (comma-separated or space-separated)",
		)
		.action(async (stationNames: string[], options, command: Command) => {
			...
			if (parsedNames.length === 0) {
				console.error(
					pc.red("Error: At least one station name must be provided"),
				);
				process.exit(2);
			}
			...
			const effect: Effect.Effect<void, PandoraError> = Effect.gen(
				function* () {
					const session = yield* ensureSession({ verbose: globalOpts.verbose });

					const stationListResponse: StationListResponse =
						yield* getStationList(session);
					const stationIds = yield* resolveStationIds(
					...
					if (!globalOpts.quiet) {
						if (globalOpts.json) {
							console.log(
								JSON.stringify(
									{ success: true, stationCount: stationIds.length },
									null,
									2,
								),
							);
							...
							console.log(
								pc.green(
									`QuickMix updated with ${stationIds.length} station${plural}`,
								),
							);
						}
					}
	...
	quickmix
		...
		.action(async (options, command: Command) => {
			...
			const effect: Effect.Effect<readonly Station[], PandoraError> =
				Effect.gen(function* () {
					const session = yield* ensureSession({ verbose: globalOpts.verbose });
			...
			if (globalOpts.json) {
				console.log(
					JSON.stringify(
						{ success: true, data: { stations: result } },
						null,
						2,
					),
				);
				...
				console.log(
					pc.dim(
						"Note: This shows all stations. The API does not return QuickMix membership status.",
					),
				);
				console.log(
					pc.dim("Use 'pandora stations list' to see all available stations."),
				);
			}
		});
}
