...
function formatUsageOutput(
  ...
  let usageBar = ""
  if (usage.accountMonthlyListening !== undefined && usage.monthlyCapHours !== undefined && usage.monthlyCapHours > 0) {
    const percent = (usage.accountMonthlyListening / usage.monthlyCapHours) * 100
    const barLength = 30
  ...
  const lines = [
    ...
    usageBar,
    "",
    formatField("Cap Warning Threshold", formatPercentage(usage.monthlyCapWarningPercent)),
    formatField("Cap Warning Repeat", formatPercentage(usage.monthlyCapWarningRepeatPercent)),
...
export function registerUsageCommand(program: Command): void {
  program
    ...
    .action(async (options, command: Command) => {
      ...
      const effect: Effect.Effect<GetUsageInfoResponse, PandoraError> = Effect.gen(function* () {
        ...
        return yield* getUsageInfo(session!)
      })
