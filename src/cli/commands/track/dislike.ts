...
async function dislikeCommand(trackToken: string, options: GlobalOptions & { station: string }): Promise<void> {
  try {
    ...
    const feedback = await Effect.runPromise(
      Client.addFeedback(session, station.stationToken, trackToken, false)
    )
    ...
    console.log(pc.yellow("Thumbs down added!"))
...
export function registerDislikeCommand(parent: Command): void {
  parent
    .command("dislike <track-token>")
    .description("Give a track a thumbs down")
    ...
    .action(async (trackToken: string, cmdOpts: { station: string }) => {
      ...
      await dislikeCommand(trackToken, { ...globalOpts, station: cmdOpts.station })
    })
}
