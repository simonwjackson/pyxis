...
export function registerCreateCommand(program: Command): void {
	program
		...
		.action(
			async (musicToken: string, options: CreateOptions, command: Command) => {
				...
				const effect: Effect.Effect<CreateStationResponse, PandoraError> =
					Effect.gen(function* () {
						...
						const response: CreateStationResponse = yield* withSession(
							(session) => createStation(session, request),
