...
const arrayMap = <T, U>(
  ...
  const out: U[] = new Array(l)
  let arrTyped: T[] = Array.isArray(arr) ? arr : []
  if (l && typeof arr === "string") arrTyped = arr.split("") as unknown as T[]
  if (thisObject) {
    for (; i < l; ++i) {
      out[i] = callback.call(thisObject, arrTyped[i]!, i, arrTyped)
    }
  } else {
    for (; i < l; ++i) {
      out[i] = callback(arrTyped[i]!, i, arrTyped)
    }
  }
  return out
}
...
let iv: BlockPair | null = null  // CBC mode initialization vector
...
const eb = (o: BlockPair, box: Box): void => {
  let l = o.left
  let r = o.right
  l = xor(l, box.p[0]!)
...
const db = (o: BlockPair, box: Box): void => {
  let l = o.left
  let r = o.right
  l = xor(l, box.p[17]!)
...
const init = (key: string | number[]): Box => {
  let k = key
  if (isString(k)) {
    k = arrayMap(k.split(""), (item) => item.charCodeAt(0) & 0xff)
  }

  let pos = 0
  let data = 0
  const res: BlockPair = { left: 0, right: 0 }
  const box: Box = {
    p: arrayMap(boxes.p.slice(0), (item) => {
      const l = k.length
      for (let j = 0; j < 4; j++) {
  ...
  for (let i = 0; i < 4; i++) {
    const sKey = ("s" + i) as "s0" | "s1" | "s2" | "s3"
    for (let j = 0, l = box[sKey].length; j < l;) {
      eb(res, box)
      box[sKey][j++] = res.left
      box[sKey][j++] = res.right
    }
  }
  return box
}

const getIV = (outputType?: number): string | number[] => {
  const out = outputType ?? outputTypes.Base64
  switch (out) {
...
const setIV = (data: string, inputType?: number): void => {
  const ip = inputType ?? outputTypes.Base64
  let ba: number[] = []
  switch (ip) {
    case outputTypes.String: {
      ba = arrayMap(data.split(""), (item) => item.charCodeAt(0))
      break
    }
    case outputTypes.Hex: {
      ba = []
  ...
  iv = {
    left: ba[0]! * POW24 | ba[1]! * POW16 | ba[2]! * POW8 | ba[3]!,
    right: ba[4]! * POW24 | ba[5]! * POW16 | ba[6]! * POW8 | ba[7]!
  }
}
...
const encrypt = (plaintext: string, key: string, ao?: BlowfishOptions): string | number[] => {
  ...
  const bx = init(key)
  let padding = 8 - (plaintext.length & 7)
  ...
  let pos = 0
  const o: BlockPair = { left: 0, right: 0 }
  const isCBC = (mode === cipherModes.CBC)
  const vector: BlockPair = { left: iv?.left ?? 0, right: iv?.right ?? 0 }
...
const decrypt = (ciphertext: string, key: string, ao?: BlowfishOptions): string => {
  ...
  switch (ip) {
    ...
    case outputTypes.String: {
      c = arrayMap(ciphertext.split(""), (item) => item.charCodeAt(0))
      break
    }
    case outputTypes.Raw: {
      c = ciphertext as unknown as number[]
      break
    }
    default: {
  ...
  return arrayMap(pt, (item) => String.fromCharCode(item)).join("")
}
