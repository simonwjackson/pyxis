...
type StationDetailArtistSeedRowProps = {
	readonly seed: StationSeed;
	readonly isRemoving: boolean;
	readonly onRemove: (seedId: string) => void;
};

export function StationDetailArtistSeedRow({
	seed,
	isRemoving,
	onRemove,
}: StationDetailArtistSeedRowProps) {
	return (
		<div className="flex items-center gap-3 p-3 bg-[var(--color-bg-highlight)] group">
			...
			<div className="flex-1 min-w-0">
				<p className="text-sm text-[var(--color-text-muted)] truncate">{seed.artistName}</p>
			</div>
			<button
				type="button"
				...
				title="Remove seed"
				aria-label={`Remove ${seed.artistName ?? "artist"} seed`}
			>
