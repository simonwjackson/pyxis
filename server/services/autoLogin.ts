 ...
 */
function mapTracksToQueue(
  tracks: ReadonlyArray<{
    readonly sourceId: { readonly source: SourceType; readonly id: string };
  ...
  QueueService.setAutoFetchHandler(async (context) => {
    ...
    if (!parsed.source) {
      logger.warn(
        ...
        "auto-fetch: seedId has no source prefix",
      );
      return [];
    }
    ...
    try {
      ...
      logger.warn(
        { code: (err as ApiCallError).code },
        "auto-fetch: Pandora auth error, refreshing session",
      );
      try {
        ...
        logger.error(
          { err: msg },
          "auto-fetch: retry after session refresh failed",
        );
        return [];
      }
    }
