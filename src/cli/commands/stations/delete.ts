...
export function registerDeleteCommand(program: Command): void {
  program
    ...
    .action(async (stationName: string, options: DeleteOptions, command: Command) => {
      ...
      const effect: Effect.Effect<string, PandoraError> = Effect.gen(function* () {
        ...
        if (!allOpts.yes && !allOpts.json) {
          const confirmed = yield* Effect.promise(() => promptConfirmation(station.stationName))
          if (!confirmed) {
            return yield* Effect.fail(
              new SessionError({
