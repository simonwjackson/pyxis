 ...
 */
function writeYamlCollection<T extends { id: string }>(
  filePath: string,
  entities: T[],
): void {
...
export async function migrateFromSqlite(): Promise<{
  ...
  try {
    // --- Migrate albums ---
    const sqliteAlbums = sqlite
      .query<SqliteAlbum, []>("SELECT * FROM albums")
      .all();
    ...
    const sqliteQueueItems = sqlite
      .query<SqliteQueueItem, []>(
        "SELECT * FROM queue_items ORDER BY queue_index ASC",
      )
      .all();
    ...
    const sqliteLogs = sqlite
      .query<SqliteListenLog, []>(
        "SELECT * FROM listen_log ORDER BY listened_at ASC",
      )
      .all();
    ...
    if (!existsSync(SQLITE_BAK_PATH)) {
      ...
      console.log(
        `Backup ${SQLITE_BAK_PATH} already exists, keeping original SQLite file`,
      );
    }
