...
export type OrchestraSourceOptions = {
	 ...
	 * Defaults to VITE_ORCHESTRA_URL env var, then 'http://aka:3847'.
	 */
	serverUrl?: string;
 ...
 */
export default function orchestraSource(
	options: OrchestraSourceOptions = {},
): Plugin {
	...
	return {
		...
		configResolved(resolvedConfig) {
			...
			serverUrl =
				options.serverUrl ?? env.VITE_ORCHESTRA_URL ?? "http://aka:3847";
 ...
 *   });
 */
export function getBabelPlugin(options: {
	root: string;
}): [string, { root: string }] {
	return [
