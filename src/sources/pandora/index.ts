...
export function isPandoraSource(source: Source): source is PandoraSource {
  return (
    source.type === "pandora" &&
    typeof (source as Partial<PandoraSource>).registerPlaylistItems ===
      "function"
  );
}
...
function resolveAudioUrl(item: PlaylistItem): string | undefined {
  ...
  if (
    Array.isArray(item.additionalAudioUrl) &&
    item.additionalAudioUrl.length > 0
  ) {
...
export function createPandoraSource(session: PandoraSession): PandoraSource {
  ...
  return {
    ...
    async listPlaylists(): Promise<readonly CanonicalPlaylist[]> {
      const result = await Effect.runPromise(Pandora.getStationList(session));
    ...
    async search(query: string) {
      const result = await Effect.runPromise(Pandora.search(session, query));
      return {
