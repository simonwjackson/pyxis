...
const log = createLogger("playback").child({ component: "android-bridge-native" });
...
const MAX_FIELDS = 16;
const SENSITIVE_KEY = /(token|secret|password|credential|streamurl|stream_url|authorization)/i;
...
export function parseNativeBridgeLogPayload(payload: unknown): ParseResult {
	...
	const allowedKeys = new Set(["event", "level", "message", "correlationId", "fields"]);
	...
	const message = boundedOptionalString(object["message"], MAX_MESSAGE_LENGTH, "message");
	if (!message.ok) return message;
	const correlationId = boundedOptionalString(object["correlationId"], 120, "correlationId");
	...
	return {
		...
		value: {
			...
			...(message.value !== undefined ? { message: redactString(message.value, "message") } : {}),
...
function boundedOptionalString(value: unknown, maxLength: number, field: string): ParseFailure | { ok: true; value?: string } {
	if (value === undefined) return { ok: true };
	if (typeof value !== "string") return { ok: false, error: `${field} must be a string` };
	if (value.length > maxLength) return { ok: false, error: `${field} too long` };
	return { ok: true, value };
}

function parseFields(value: unknown): ParseFailure | { ok: true; value?: Record<string, string | number | boolean | null> } {
	if (value === undefined) return { ok: true };
	if (!value || typeof value !== "object" || Array.isArray(value)) {
		return { ok: false, error: "fields must be an object" };
	}
	const entries = Object.entries(value as Record<string, unknown>);
	if (entries.length > MAX_FIELDS) return { ok: false, error: "too many fields" };
	...
	for (const [key, raw] of entries) {
		...
		if (raw.length > MAX_FIELD_LENGTH) return { ok: false, error: "field value too long" };
