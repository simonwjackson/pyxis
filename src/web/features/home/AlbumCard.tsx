...
export const AlbumCard = memo(function AlbumCard({ album }: AlbumCardProps) {
  return (
    ...
    >
      <div className="aspect-square bg-pyxis-highlight mb-2 relative overflow-hidden">
        {album.artworkUrl ? (
          <img
          ...
          <div className="w-full h-full flex items-center justify-center">
            <Disc3 className="w-12 h-12 text-pyxis-dim" />
          </div>
        )}
        <div className="absolute top-2 left-2 flex gap-1 flex-wrap">
          <span
            className={`zune-badge px-1.5 py-0.5 ${placementBadgeClassName(album.placement)}`}
          >
