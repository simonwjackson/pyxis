 ...
 */
const normalizeAutocompleteItem = (
    ...
    artists: [
      {
        name: artistName,
        ids: item.band_id
          ? [{ source: "bandcamp", id: String(item.band_id) }]
          : [],
      },
    ],
    releaseType,
 ...
 */
const decodeCompositeId = (
  compositeId: string,
): { readonly bandId: number; readonly itemId: number } => {
...
export const createBandcampFullSource = (
  ...
  const getAlbumTracks = async (albumId: string) => {
    ...
    const albumArtist =
      details.tralbum_artist ?? details.band?.name ?? "Unknown";
    const artworkUrl =
      details.art_id != null
  ...
  const getStreamUrl = async (trackId: string): Promise<string> => {
    ...
    const streamUrl =
      track.streaming_url != null && typeof track.streaming_url === "object"
