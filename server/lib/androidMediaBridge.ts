...
import type { AndroidMediaBridgeCommandResult, AndroidMediaBridgeState } from "../../src/api/contracts/androidMediaBridge.js";
import { toPlayerStateView } from "./playerStateView.js";
import { toAndroidMediaBridgeState } from "./androidMediaBridgeState.js";
import { parseNativeBridgeLogPayload, writeNativeBridgeLog } from "./androidMediaBridgeLog.js";
...
export function isAndroidMediaBridgeRequest(url: URL): boolean {
	return url.pathname === BRIDGE_PREFIX || url.pathname.startsWith(`${BRIDGE_PREFIX}/`);
}

export function createAndroidMediaBridge(config: AndroidMediaBridgeConfig): AndroidMediaBridgeHandler {
...
export class AndroidMediaBridgeHandler {
	...
	private readonly commandLimit: { readonly max: number; readonly windowMs: number };
	...
	private authorize(req: Request): Response | null {
		...
		if (provided !== this.token) {
			log.warn({ method: req.method, path: new URL(req.url).pathname, origin: req.headers.get("origin") ?? "none" }, "bridge unauthorized");
	...
	private eventFeed(): Response {
		...
		const stream = new ReadableStream<Uint8Array>({
			start: (controller) => {
				const sendState = () => {
					controller.enqueue(encoder.encode(formatSse("state", this.projectState())));
				};
				sendState();
	...
	private async command(req: Request): Promise<Response> {
		if (!this.consumeCommandSlot()) {
			log.warn({ path: new URL(req.url).pathname }, "bridge command rate limited");
		...
		switch (command.value.action) {
			...
			case "next": {
				...
				outcome = next || PlayerService.getState().status === "stopped" ? "applied" : "unavailable";
				break;
			}
			case "previous": {
				const previous = before.currentTrack ? PlayerService.previousTrack() : undefined;
		...
		log.info({ action: command.value.action, outcome, source: command.value.source, correlationId: command.value.correlationId }, "bridge command");
...
function parseCommand(payload: unknown):
	| { readonly ok: true; readonly value: { readonly action: "play" | "pause" | "next" | "previous"; readonly correlationId: string; readonly source: string } }
	...
	for (const key of Object.keys(object)) {
		if (!allowedKeys.has(key)) return { ok: false, error: `unknown field: ${key}` };
	}
	const action = object["action"];
	if (action !== "play" && action !== "pause" && action !== "next" && action !== "previous") {
		return { ok: false, error: "invalid action" };
	}
	const correlationId = typeof object["correlationId"] === "string" && object["correlationId"].length <= 120
		? object["correlationId"]
		: crypto.randomUUID();
	const source = typeof object["source"] === "string" && object["source"].length <= 120 ? object["source"] : "android";
	return { ok: true, value: { action, correlationId, source } };
}

async function readJson(req: Request, maxBytes: number): Promise<{ ok: true; value: unknown } | { ok: false; error: string }> {
