...
describe.skipIf(!shouldRunPandoraFixtures)("user API", () => {
  ...
  describe("getUsageInfo", () => {
    ...
    it("should validate listening cap relationships", async () => {
      ...
      if (
        result.accountMonthlyListening !== undefined &&
        result.monthlyCapHours !== undefined
      ) {
      ...
      if (
        result.monthlyCapWarningPercent !== undefined &&
        result.monthlyCapWarningRepeatPercent !== undefined
      ) {
        // Repeat warning should be at or above initial warning
        expect(result.monthlyCapWarningRepeatPercent).toBeGreaterThanOrEqual(
          result.monthlyCapWarningPercent,
        );
      }
    });
  });
  ...
  describe("getStationListChecksum", () => {
    it("should return checksum from fixture", async () => {
      const result = await expectEffectSuccess(
        getStationListChecksum(mockAuthState),
      );
  ...
  describe("setQuickMix", () => {
    it("should successfully set quick mix stations", async () => {
      ...
      const result = await expectEffectSuccess(
        setQuickMix(mockAuthState, request),
      );
  ...
  describe("changeSettings", () => {
    it("should successfully change single setting", async () => {
      ...
      const result = await expectEffectSuccess(
        changeSettings(mockAuthState, request),
      );
  ...
  describe("setExplicitContentFilter", () => {
    it("should enable explicit content filter", async () => {
      ...
      const result = await expectEffectSuccess(
        setExplicitContentFilter(mockAuthState, request),
      );
  ...
  describe("sleepSong", () => {
    it("should successfully sleep a song", async () => {
      ...
      const result = await expectEffectSuccess(
        sleepSong(mockAuthState, request),
      );
    ...
    it("should handle different track tokens", async () => {
      ...
      const result1 = await expectEffectSuccess(
        sleepSong(mockAuthState, request1),
      );
      const result2 = await expectEffectSuccess(
        sleepSong(mockAuthState, request2),
      );
