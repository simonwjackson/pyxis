...
describe("buildStreamUrl", () => {
  ...
  it("builds stream url with both next hint and format", () => {
    ...
    expect(url).toBe(
      "/stream/pandora%3Aabc123?next=pandora%3Anext456&format=mp3",
    );
  });
...
describe("resolveTrackForStream", () => {
  ...
  it("throws for unknown bare nanoid when DB lookup fails", async () => {
    ...
    await expect(resolveTrackForStream("unknownId1")).rejects.toThrow(
      "Unknown track ID",
    );
  });
});
...
describe("resolveTrackSource", () => {
  ...
  it("throws for unknown bare nanoid when DB lookup fails", async () => {
    await expect(resolveTrackSource("unknownId2")).rejects.toThrow(
      "Unknown track ID",
    );
  });
});
