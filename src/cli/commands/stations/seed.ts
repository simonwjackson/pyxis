...
function formatAddMusicResponse(
  ...
  if (response.artistName && response.songName) {
    output += "\n" + pc.bold("Song") + ': "' + response.songName + '" by ' + response.artistName
...
function formatDeleteMusicResponse(
  options: { json: boolean }
): string {
  if (options.json) {
    const result = {
...
function registerAddCommand(program: Command): void {
  program
    ...
    .action(async (stationName: string, musicToken: string, command: Command) => {
      const parentCommand = command.parent as Command & {
        parent?: Command & {
          parent?: Command & { optsWithGlobals?: () => GlobalOptions }
        }
      }
      const globalOpts: GlobalOptions = parentCommand.parent?.parent?.optsWithGlobals?.() ?? {
        json: false,
        cache: true,
        verbose: false,
        quiet: false,
      ...
      const effect: Effect.Effect<AddMusicResponse, PandoraError> = Effect.gen(function* () {
  ...
  program
    ...
    .action(async (seedId: string, command: Command) => {
      ...
      const effect: Effect.Effect<Record<string, never>, PandoraError> = Effect.gen(function* () {
        ...
        // Delete the music seed
        const response: Record<string, never> = yield* deleteMusic(session, {
          seedId,
        })
