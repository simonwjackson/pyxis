...
import { expectEffectFailure, expectEffectSuccess } from "../test-utils.js";
import { EncryptionError } from "../types/errors.js";
import { decrypt, encrypt, encryptJson } from "./index.js";

describe("crypto", () => {
  const testKey = ANDROID_DEVICE.encryptKey;
  const _decryptKey = ANDROID_DEVICE.decryptKey;
  ...
  describe("encrypt", () => {
    ...
    it("should produce different output for different keys", async () => {
      const result1 = await expectEffectSuccess(
        encrypt(testKey)("same message"),
      );
      const result2 = await expectEffectSuccess(
        encrypt("differentkey")("same message"),
      );
