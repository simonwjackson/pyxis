...
function parseBooleanArg(value: string): boolean {
  ...
  throw new Error(`Invalid boolean value: ${value}. Use: on/off, true/false, yes/no, or 1/0`)
}
  ...
  set
    ...
    .action(async (value: string, options, command: Command) => {
      ...
      const effect: Effect.Effect<void, PandoraError> = Effect.gen(function* () {
        ...
        yield* setExplicitContentFilter(session!, enabled)
      ...
      const effect: Effect.Effect<void, PandoraError> = Effect.gen(function* () {
        ...
        if (!globalOpts.quiet) {
          if (globalOpts.json) {
            ...
            const status = isPrivate ? pc.green('private') : pc.yellow('public')
