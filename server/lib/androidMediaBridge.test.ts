...
import { createAndroidMediaBridge, isAndroidMediaBridgeRequest } from "./androidMediaBridge.js";
...
describe("android media bridge HTTP boundary", () => {
	...
	it("recognizes only bridge paths", () => {
		expect(isAndroidMediaBridgeRequest(new URL("http://x/android-media-bridge/state"))).toBe(true);
		expect(isAndroidMediaBridgeRequest(new URL("http://x/trpc/player.getState"))).toBe(false);
	});
	...
	it("returns an authorized state snapshot", async () => {
		...
		const body = await json(response) as { currentTrack: { title: string }; availableActions: string[] };
	...
	it("rejects missing or invalid tokens before command payload changes state", async () => {
		...
		const response = await bridge.handle(request("/android-media-bridge/commands", {
			method: "POST",
	...
	it("routes authorized pause and play commands through player authority", async () => {
		...
		const pauseResponse = await bridge.handle(request("/android-media-bridge/commands", {
			method: "POST",
			headers: { "x-pyxis-bridge-token": "secret", "content-type": "application/json" },
			body: JSON.stringify({ action: "pause", correlationId: "pause-1", source: "test" }),
		}));
		const pauseBody = await json(pauseResponse) as { outcome: string; state: { status: string } };
		...
		const playResponse = await bridge.handle(request("/android-media-bridge/commands", {
			method: "POST",
			headers: { "x-pyxis-bridge-token": "secret", "content-type": "application/json" },
			body: JSON.stringify({ action: "play", correlationId: "play-1", source: "test" }),
		}));
		const playBody = await json(playResponse) as { outcome: string; state: { status: string } };
	...
	it("emits initial and subsequent state events", async () => {
		...
		const response = await bridge.handle(request("/android-media-bridge/events", {
			headers: { "x-pyxis-bridge-token": "secret" },
		}));
	...
	it("bounds command storms", async () => {
		PlayerService.play([track()], { type: "manual" });
		const bridge = createAndroidMediaBridge({ enabled: true, token: "secret", now: () => 1001, commandLimit: { max: 1, windowMs: 10_000 } });
		const init = {
			method: "POST",
			headers: { "x-pyxis-bridge-token": "secret", "content-type": "application/json" },
			body: JSON.stringify({ action: "pause" }),
		...
		const second = await bridge.handle(request("/android-media-bridge/commands", init));
