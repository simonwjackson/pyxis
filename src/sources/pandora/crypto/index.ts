...
import { DecryptionError, EncryptionError } from "../types/errors.js";
...
export const encrypt =
  (key: string) =>
  (text: string): Effect.Effect<string, EncryptionError> =>
    Effect.try({
      try: () => {
        ...
        new EncryptionError({ message: "Failed to encrypt", cause }),
    });
...
export const encryptJson =
  (key: string) =>
  (data: unknown): Effect.Effect<string, EncryptionError> =>
    Effect.try({
      try: () => JSON.stringify(data),
      catch: (cause) =>
        new EncryptionError({ message: "Failed to serialize JSON", cause }),
    }).pipe(Effect.flatMap(encrypt(key)));
...
export const decrypt =
  (key: string) =>
  (text: string): Effect.Effect<string, DecryptionError> =>
    Effect.try({
      try: () =>
        blowfish.decrypt(text, key, {
          cipherMode: CIPHER_MODE_ECB,
          outputType: OUTPUT_TYPE_HEX,
        }),
      catch: (cause) =>
        new DecryptionError({ message: "Failed to decrypt", cause }),
    });
