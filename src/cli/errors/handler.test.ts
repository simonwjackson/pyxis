...
describe("CLI Error Handler", () => {
	...
	describe("formatCliError - Human Readable Format", () => {
		...
		it("should format ConfigError with init hint", () => {
			...
			expect(output).toContain("pyxis config init");
		});
		...
		it("should format SessionError with login hint", () => {
			...
			expect(output).toContain("pyxis auth login");
		});
	});
	...
	describe("handleEffectError", () => {
		...
		it("should handle PandoraError and exit with correct code", () => {
			...
			try {
				...
				// Expected - handleEffectError throws after process.exit when exit is mocked
			}
		...
		it("should handle errors with all different exit codes", () => {
			const testCases = [
				{
					error: new EncryptionError({ message: "E" }),
					code: EXIT_CODE.GENERAL_ERROR,
				},
				{
					error: new DecryptionError({ message: "D" }),
					code: EXIT_CODE.GENERAL_ERROR,
				},
				{
					error: new PartnerLoginError({ message: "P" }),
					code: EXIT_CODE.NETWORK_ERROR,
				},
				{
					error: new UserLoginError({ message: "U" }),
					code: EXIT_CODE.AUTH_FAILED,
				},
				{
					error: new ApiCallError({ method: "m", message: "A" }),
					code: EXIT_CODE.API_ERROR,
				},
				{
					error: new ConfigError({ message: "C" }),
					code: EXIT_CODE.CONFIG_ERROR,
				},
				{
					error: new NotFoundError({ message: "N" }),
					code: EXIT_CODE.RESOURCE_NOT_FOUND,
				},
				{
					error: new SessionError({ message: "S" }),
					code: EXIT_CODE.AUTH_FAILED,
				},
			];
